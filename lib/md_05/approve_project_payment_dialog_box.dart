...
class ApproveConstructionDialog extends StatefulWidget {
  ...
  @override
  ApproveConstructionDialogState createState() => ApproveConstructionDialogState();
}
...
class ApproveConstructionDialogState extends State<ApproveConstructionDialog> {
  ...
  Future<void> _loadRequestInfo() async {
    try {
      ...
      ExceptionLogger.logToError(message: e.toString(),errorLog: st.toString(), logFile: 'approve_project_payment_dialog_box.dart');
      Navigator.pop(context);
  ...
  Future<void> approveOrRejectRequest(BuildContext context, int requestId, int isApproved, String comment) async {
    ...
    try {
      ...
      String apiURL='${APIHost().apiURL}/project_payment_controller.php/Approve';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
