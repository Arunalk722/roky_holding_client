...
class AuthorizationOfzRequestDialog extends StatefulWidget {
  ...
  @override
  AuthorizationOfzRequestDialogState createState() => AuthorizationOfzRequestDialogState();
}

class AuthorizationOfzRequestDialogState extends State<AuthorizationOfzRequestDialog> {
  late bool isToCEO;
  ...
  Future<void> _loadRequestInfo() async {
    try {
      ...
      ExceptionLogger.logToError(message: e.toString(),errorLog: st.toString(), logFile: 'auth_ofz_request_dialog_box.dart');
      Navigator.pop(context);
  ...
  void initState() {
    ...
    isToCEO = false; // Initialize default state
  ...
  Future<void> approveOrRejectRequest(BuildContext context, int requestId, int isAuthorized, String comment) async {
    ...
    try {
      String apiURL ='${APIHost().apiURL}/ofz_payment_controller.php/Authorized';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
