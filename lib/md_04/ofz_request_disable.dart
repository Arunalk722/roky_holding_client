...
class DisablingOfzRequestDialog extends StatefulWidget {
  ...
  @override
  DisablingOfzRequestDialogState createState() => DisablingOfzRequestDialogState();
}
...
class DisablingOfzRequestDialogState extends State<DisablingOfzRequestDialog> {
  ...
  Future<void> _loadRequestInfo() async {
    try {
      ...
      ExceptionLogger.logToError(message: e.toString(),errorLog: st.toString(), logFile: 'ofz_request_disable.dart');
  ...
  Future<void> _disableRequest(BuildContext context, String requestId, bool confirmToDisable, String comment) async {
    ...
    try {
      ...
      String reqUrl='${APIHost().apiURL}/ofz_payment_controller.php/DisableRequest';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
